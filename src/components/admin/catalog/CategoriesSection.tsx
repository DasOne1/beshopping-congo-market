...
const CategoriesSection = ({ searchTerm }: CategoriesSectionProps) => {
  ...
  // Séparer les catégories visibles et masquées
  ...
  const renderCategoryTable = (categoryList: any[], showVisibilityToggle = true) => (
        ...
        <CardContent>
          <Tabs defaultValue="visible" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="visible">Catégories Visibles ({visibleCategories.length})</TabsTrigger>
              <TabsTrigger value="hidden">Catégories Masquées ({hiddenCategories.length})</TabsTrigger>
            </TabsList>
            
            <TabsContent value="visible" className="mt-6">
              {visibleCategories.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  Aucune catégorie visible trouvée
                </div>
              ) : (
                renderCategoryTable(visibleCategories, true)
              )}
            </TabsContent>
            ...
            <TabsContent value="hidden" className="mt-6">
              {hiddenCategories.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  Aucune catégorie masquée trouvée
                </div>
              ) : (
                renderCategoryTable(hiddenCategories, true)
              )}
            </TabsContent>
          </Tabs>
        </CardContent>
